...
const schemaValidation = z.object({
  ...
  product_ids: z
    .array(
      z
        .string()
        .refine((value) => Types.ObjectId.isValid(value), "Invalid product ID"),
    )
...
export default async function (req, res) {
  try {
    ...
    const findUserId = await userModel.findOne({
      _id: checkValidation.data.user_id,
      deleted_at: null,
    });

    if (!findUserId) return message(res, 404, "User tidak ditemukan");

    const findProductId = await productModel.find({
      _id: { $in: checkValidation.data.product_ids },
    });
